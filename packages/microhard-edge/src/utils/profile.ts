...
import { getEdgeDataPath, getLocalStatePath, getWorkspaceCacheFilePath } from "./edgePaths";
...
const loadProfileNames = (): Map<string, string> => {
  ...
  try {
    ...
    if (profiles) {
      Object.entries<{ name: string; user_name: string }>(profiles).forEach(([key, val]) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        const displayName = profileNames.get(item) || (isDefaultProfile ? "Default" : item);
